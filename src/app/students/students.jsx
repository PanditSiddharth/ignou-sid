...
const StudentsTab = ({
  ...
  const loadMore = useCallback(async () => {
    ...
    const handleActiveStudent = (e, index) => {
      ...
      return (
        <div className={"flex flex-col pt-2 w-full bg-white dark:bg-sky-800 overflow-auto "} id="students" style={{ height: 'calc(100vh - 6.7rem)' }} >
          <InfiniteScroll
            pageStart={0}
            initialLoad={false}
            loadMore={loadMore}
            hasMore={stData[mkey].totalCount >= (stData[mkey].page * 20)}
            loader={<div className="h-10 w-full text-center" key={0}>Loading ...</div>}
          ...
          >
            ...
            {stData[mkey].totalCount < (stData[mkey].page * 20) && <div className='h-10 w-full text-center'>No more students</div>}
          </InfiniteScroll>
        </div>
      )
    };
